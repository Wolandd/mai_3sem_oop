...
public:
    ...
    Triangle() : points(VERTEX_COUNT, std::make_pair(0.0, 0.0)) {}

    Triangle(const std::vector<std::pair<double, double>>& points_list) {
    ...
    Triangle(const Triangle& src) : points(src.points) {}
    Triangle(Triangle&& src) noexcept : points(std::move(src.points)) {}

    Figure& operator=(const Figure& rhs) override {
        if (this == &rhs) return *this;
        const Triangle* triangle = dynamic_cast<const Triangle*>(&rhs);
        if (!triangle) {
            throw std::invalid_argument("Cannot assign non-Triangle to Triangle");
        }
    ...
    Figure& operator=(Figure&& rhs) noexcept override {
        Triangle* triangle = dynamic_cast<Triangle*>(&rhs);
        if (!triangle) {
            return *this;
        }
    ...
    bool operator==(const Figure& rhs) const override {
        ...
        for (size_t idx = 0; idx < VERTEX_COUNT; ++idx) {
            if (std::abs(points[idx].first - triangle->points[idx].first) >= tolerance ||
                std::abs(points[idx].second - triangle->points[idx].second) >= tolerance)
                return false;
        }
        return true;
    }
